...
#define		SYSTEM_PART_SIZE		SDMMC_SYSTEM_PART_SIZE
#define		USER_DATA_PART_SIZE		SDMMC_USER_DATA_PART_SIZE	
#define		CACHE_PART_SIZE			SDMMC_CACHE_PART_SIZE
...
/**
	return 1 if sdcard special,return 0 if normal
*/
char	special_sig[] = "Special Sdcard for Sangu";
int check_special_sdcard()
{
	char	mbr[512];
	int	ret = get_mmc_mbr("1", mbr);
	if(ret < 0 )
		return 0;
	
	int	len = strlen(special_sig);
	if( strncmp(mbr,special_sig,len) == 0)
	{
		printf("special sdcard found\n");
		return 1;
	}
	return 0;
}
...
int mbr_validation()
{
	char	mbr_new[512],mbr_old[512];
	memset(mbr_new,0x00,512);
	memset(mbr_old,0x00,512);

	get_mmc_mbr("0",mbr_old);
	int	total = get_mmc_block_count("0");
	if(total < 0)
		return -1;
	
	make_mmc_partition(total, mbr_new);
	if(memcmp(mbr_new,mbr_old,512) != 0)
	{
		printf("mbr damaged,not try to recover\n");
		//put_mmc_mbr(mbr_new, "0");
	}
	return 0;
}
/////////////////////////////////////////////////////////////////
