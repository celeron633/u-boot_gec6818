...
//wky for recovery
		...
		//image[6].start_blk = image[5].start_blk + (PART_SIZE_ROOTFS/MOVI_BLKSIZE);
		image[6].start_blk = ((7*1024*1024)/MOVI_BLKSIZE);//7M as start
		image[6].used_blk = (2 * 1024 * 1024);
		image[6].size = (2* 1024 * 1024);
		image[6].attribute = 0x20;
		strcpy(image[6].description, "cover");
		dbg("cover:%d\n",image[6].start_blk);
...
//zhf for bootup logo
		...
		/* for boot logo */
		image[7].start_blk = ((9*1024*1024)/MOVI_BLKSIZE);
		image[7].used_blk = (2 * 1024 * 1024/MOVI_BLKSIZE);
		image[7].size = (2 * 1024 * 1024);
		image[7].attribute = 0x40;
		strcpy(image[3].description, "bootlogo");
		dbg("logo: %d\n", image[3].start_blk);
#endif
...
#define	BL2_SIZE 8192
	...
	if(attribute == 0x20)
	{
		...
		blkcnt = rfs_size/MOVI_BLKSIZE +
			((rfs_size&(MOVI_BLKSIZE-1)) ? 1 : 0);
		image[i].used_blk = blkcnt;
		printf("%s recovery.. %ld, %ld ", rw ? "writing":"reading",
				start_blk, blkcnt);
		sprintf(run_cmd,"mmc %s 0 0x%lx 0x%lx 0x%lx",
				rw ? "write":"read",
				addr, start_blk, blkcnt);
