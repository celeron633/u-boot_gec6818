...
#define MAXIMUM_BLOCK_COUNT 0x2000
...
int mmc_write_progress=0x31415926;
	...
	default: /* at least 5 args */
		if (strcmp(argv[1], "read") == 0) {
			...
			do {
				...
				mmc_write_progress = 100*n/cnt;
				sprintf(response, "INFO PROGRESS:%d",mmc_write_progress);
				fastboot_tx_status(response, strlen(response), FASTBOOT_TX_SYNC);//qdk++
				printf("%s\r",response);
				//fastboot_poll();//qdk++
			} while(cnt > n);
