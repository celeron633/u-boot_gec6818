...
void s5p_lcd_draw_boot_logo(const unsigned char * buf)
{
	...
	sx = (1024-BOOT_LOGO_BITMAP_WIDTH)>>1;
	sy = (600-BOOT_LOGO_BITMAP_HEIGHT)>>1;
...
void main_loop (void)
{
    ...
    //run_command("sdfuse flash bootloader u-boot.bin", 0);
...
#ifdef CFG_RECOVERY
		//recovery_preboot();  //wky for recovery
		//keypad_test();
#endif
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
...
#if 0
	writel((readl(ELFIN_GPIO_BASE+GPC1CON_OFFSET)&(~0xF0))|0x10, ELFIN_GPIO_BASE+GPC1CON_OFFSET); 
	writel(readl(ELFIN_GPIO_BASE+GPC1DAT_OFFSET)|(1<<1), ELFIN_GPIO_BASE+GPC1DAT_OFFSET); 

	for (i = 0; i < 100; i++)
		udelay(10000);
 
	writel(readl(ELFIN_GPIO_BASE+GPC1DAT_OFFSET)& (~(1<<1)), ELFIN_GPIO_BASE+GPC1DAT_OFFSET); 
	writel((readl(ELFIN_GPIO_BASE+GPC1CON_OFFSET)&(~0xF0)), ELFIN_GPIO_BASE+GPC1CON_OFFSET);
#endif
#ifdef SHOW_BOOT_LOGO
	char run_cmd[100];
	...
	start_blk = ((9*1024*1024)/MOVI_BLKSIZE);
	...
	memset((void*)CFG_LCD_FBUFFER, 0x00, 1366*600*4);
	LCD_turnon(0);
