...
void s5p_lcd_draw_bitmap(const unsigned char * buf)
{
	...
	sx = (800-BAT_CHG_BITMAP_WIDTH)>>1;
	sy = (500-BAT_CHG_BITMAP_HEIGHT)>>1;
	sy -= 20;
...
CHARGING:
	...
	if (readl(INF_REG_BASE+INF_REG0_OFFSET)!=0xFFAADDEE)
	{
		...
		{
			s5p_iic0_write(0xB6, 0x78, 0x30); //acin still keeps eoc irq, when plug out the acin, make it display charged fully
		}
