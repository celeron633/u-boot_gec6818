...
#define	CONSOLE_END	(CONSOLE_ROW_FIRST + CONSOLE_SIZE)
...
static void video_drawchars (int xx, int yy, unsigned char *s, int count)
{
	...
	offset = (VIDEO_VISIBLE_ROWS-yy-8) * VIDEO_LINE_LEN + (VIDEO_VISIBLE_COLS-xx-VIDEO_FONT_WIDTH) * VIDEO_PIXEL_SIZE;
	...
	case GDF_32BIT_X888RGB:
		while (count--) {
			...
			for (rows = VIDEO_FONT_HEIGHT, dest = dest0;
				...
				((u32 *) dest)[7] = SWAP32 ((video_font_draw_table32 [bits >> 4][0] & eorx) ^ bgx);
				((u32 *) dest)[6] = SWAP32 ((video_font_draw_table32 [bits >> 4][1] & eorx) ^ bgx);
				((u32 *) dest)[5] = SWAP32 ((video_font_draw_table32 [bits >> 4][2] & eorx) ^ bgx);
				((u32 *) dest)[4] = SWAP32 ((video_font_draw_table32 [bits >> 4][3] & eorx) ^ bgx);
				((u32 *) dest)[3] = SWAP32 ((video_font_draw_table32 [bits & 15][0] & eorx) ^ bgx);
				((u32 *) dest)[2] = SWAP32 ((video_font_draw_table32 [bits & 15][1] & eorx) ^ bgx);
				((u32 *) dest)[1] = SWAP32 ((video_font_draw_table32 [bits & 15][2] & eorx) ^ bgx);
				((u32 *) dest)[0] = SWAP32 ((video_font_draw_table32 [bits & 15][3] & eorx) ^ bgx);
			}
			dest0 -= VIDEO_FONT_WIDTH * VIDEO_PIXEL_SIZE;
			s++;
		}
...
static void d_memsetl(int *p, int c, int v)
{
	while (c--)
		*(p--) = v;
}
...
static void d_memcpyl (int *d,int *s, int c)
{
	while (c--)
		*(d--) = *(s--);	
}
...
static void console_scrollup (void)
{
	...
	d_memcpyl(CONSOLE_END,CONSOLE_ROW_LAST,CONSOLE_SCROLL_SIZE >> 2);
#endif
	...
	d_memsetl(CONSOLE_ROW_SECOND,CONSOLE_ROW_SIZE >> 2,CONSOLE_BG_COL);
#endif
}
