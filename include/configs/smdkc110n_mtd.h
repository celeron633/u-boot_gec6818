...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"fuse_uboot=fatload mmc 0:1 40000000 images/${LINUXBOOTLOADER};onenand erase 0 ${filesize};onenand write 40000000 0 ${filesize}\0" \
	"fuse_kernel=fatload mmc 0:1 40000000 images/${LINUXKERNEL};onenand erase 600000 ${filesize};onenand write 40000000 600000 ${filesize}\0" \
	"fuse_rootfs=fatload mmc 0:1 38000000 images/${LINUXROOTFSINSTALLIMAGE};onenand erase e00000 ${filesize};onenand write.yaffs 38000000 e00000 ${filesize}\0" \
	"fuse_onenand=" \
		"buzzer play 1; led on; " \
		"lcd Writing\\ U-boot; run fuse_uboot; " \
		"lcd Writing\\ Kernel; run fuse_kernel; " \
		"lcd Writing\\ RootFS; run fuse_rootfs; " \
		"buzzer play 2; " \
		"lcd Finish; while test $bootdev = sd; do led play; done;\0" \
	""
