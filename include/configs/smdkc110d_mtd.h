...
#define CONFIG_VIDEO_LOGO_ADDRESS CFG_LCD_FBUFFER + 0x500000
#define	SDMMC_SYSTEM_PART_SIZE		(300*1024*1024)
#define	SDMMC_USER_DATA_PART_SIZE	(1000*1024*1024)	
#define	SDMMC_CACHE_PART_SIZE		(400*1024*1024)	
 ...
 */
#define CONFIG_SERIAL1          1	/* we use SERIAL 2 for Universal b'd*/
...
#define CFG_PROMPT              "TinyC110 # "   /* Monitor Command Prompt       */
...
/* s5pc110d: 3 bank of DRAM (80MB OneDRAM + 256MB mDDR + 128MB mDDR)
 * s5pc110n: 2 bank of DRAM (256MB mDRAM + 256MB mDDR)
 * so the max banks is 3.
 */
...
#define CFG_FASTBOOT_TRANSFER_BUFFER_SIZE	(0x9000000)	/* 144MB */
...
#define CONFIG_FUSE_UBOOT \
	"echo fusing u-boot; " \
	"setenv fuse fail; " \
	"if test -n ${LINUXBOOTLOADER}; then " \
		...
		"fi; " \
	"fi; " \
	"if test ${fuse} = fail; then echo fail to fuse u-boot; fi; "

#define CONFIG_FUSE_KERNEL \
	"echo fusing kernel; " \
	"setenv fuse fail; " \
	"if test -n ${LINUXKERNEL}; then " \
		...
		"fi; " \
	"fi; " \
	"if test ${fuse} = fail; then echo fail to fuse kernel; fi; "

#define CONFIG_FUSE_ROOTFS \
	"echo fusing rootfs; " \
	"setenv fuse fail; " \
	"if test -n ${LINUXROOTFSINSTALLIMAGE}; then " \
		"fatload mmc 0:1 ${fuse_addr} images/${LINUXROOTFSINSTALLIMAGE}; " \
		"if test $? -eq 0; then " \
			"if test ${filesize} -gt 0; then " \
		...
		"fi; " \
	"fi; " \
	"if test ${fuse} = fail; then echo fail to fuse rootfs; fi; "

#define CONFIG_FUSE_ONENAND \
	"buzzer play 1; led on; " \
	"lcd Writing\\ U-boot; run fuse_uboot; " \
	"if test ${fuse} != fail; then lcd Writing\\ Kernel; run fuse_kernel;  fi; " \
	"if test ${fuse} != fail; then lcd Writing\\ RootFS; run fuse_rootfs; fi; " \
	"if test ${fuse} = fail; then " \
		"buzzer play 3; " \
		"echo Install Linux failed; " \
		"lcd Install\\ Linux\\ failed; " \
		"led off; " \
	"else; " \
		"buzzer play 2; " \
		"echo Install Linux OK. Please reboot to Nandboot; " \
		"lcd Install\\ Linux\\ OK; " \
		"while test ${fuse} = ok; do led play; done; " \
	"fi;"
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"fuse_onenand="CONFIG_FUSE_ONENAND"\0" \
	""
#define CONFIG_AUTO_COMPLETE	1       /* add autocompletion support   */

#define CONFIG_FRIENDLYELEC

#if defined(CONFIG_FRIENDLYELEC)
#ifndef __ASSEMBLY__
extern int s5pc110_chip;
extern int s5cp110_dram_banks;
#define CONFIG_NR_DRAM_BANKS (s5cp110_dram_banks)
enum S5PC110_CHIP_TYPE {
	S5PC110_CHIP_TYPE_INVALID = 0,
	S5PC110_CHIP_TYPE_D,
	S5PC110_CHIP_TYPE_N,
};
#endif
