...
void IIC_EAck()
{
	...
	//qdk--ack = (ack>>4)&0x1;
	ack = ack&0x1;//qdk++ we use channel 0
	while(ack!=0);
...
void IIC_ESetport(void)
{
	...
	GPD1PUD &= ~0xf;	// Pull Up/Down Disable	SCL, SDA
...
#endif
	IIC_EWrite(TPS659101_ADDR, 0x3F, 1<<4);
	//VPLL
	IIC_EWrite(TPS659101_ADDR, 0x36, 0x05);
	//VDD1
	//IIC_EWrite(TPS659101_ADDR, 0x22, 0x3C);
	IIC_EWrite(TPS659101_ADDR, 0x22, 0x35);
	//VDD2
	IIC_EWrite(TPS659101_ADDR, 0x25, 0x2B);
	//VDIG2
	IIC_EWrite(TPS659101_ADDR, 0x31, 0x05);
	//VDD_MEM
	IIC_EWrite(TPS659101_ADDR, 0x20, 0x05);
	//VMMC
	IIC_EWrite(TPS659101_ADDR, 0x35, 0x0D);
	//
	IIC_EWrite(TPS659101_ADDR, 0x37, 0x0D);
	//IIC_EWrite(TPS659101_ADDR, 0x42, 0x06);
	//VDD_ADC
	IIC_EWrite(TPS659101_ADDR,0x33,0x09); //turn on vdd_adc
}
