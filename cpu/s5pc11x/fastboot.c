...
//the i-RAM range from d0020000 to d0037fff
//we use d0036000-d0037fff to pass message between stages
...
#define BOOT_MSG_ADDR	0xd0036000
#define BOOT_MSG_MAGIC	0x31415926
#define BOOT_TYPE_BOOTLOADER 1
#define BOOT_TYPE_RECOVERY 2
#define BOOT_TYPE_NORMAL 0
void 	init_boot_msg()
{
	struct _boot_msg* bmsg = (struct _boot_msg*)BOOT_MSG_ADDR;
	if(bmsg->magic != BOOT_MSG_MAGIC){
		bmsg->magic = BOOT_MSG_MAGIC;
		bmsg->boot_type = BOOT_TYPE_NORMAL;
		bmsg->power_mode = 0;
		bmsg->security = 0;
		memset(bmsg->bm.command,0,32);
		memset(bmsg->bm.status,0,32);
		memset(bmsg->bm.recovery,0,1024);
	}
}

#define S5PV210_GPJ2_BASE 0xE0200280
#define S5PV210_GPJ3_BASE 0xE02002A0
#define S5PV210_GPJ4_BASE 0xE02002C0



//we have 3 keys to be configured
//GPJ2(4) GPJ2(5) should be pull down
//GPH0(3) should be pull up
void init_gpiokey()
{
	int dat;
	
	//set input
	dat = readl(S5PV210_GPJ2_BASE+0);
	dat &= ~0xFF0000;
	writel(dat,S5PV210_GPJ2_BASE);
	
	//set pull down
	dat = readl(S5PV210_GPJ2_BASE+8);
	dat &=~0xF00;
	dat |=0x500;
	writel(dat,S5PV210_GPJ2_BASE+8);

	dat = readl(S5PV210_GPJ3_BASE);
	dat &= ~0xF0FF00;
	writel(dat,S5PV210_GPJ3_BASE);
	
	//set pull up
	dat = readl(S5PV210_GPJ3_BASE+8);
	dat &=~0xC0;
	dat |=0x80;
	writel(dat,S5PV210_GPJ3_BASE+8);	
}
#define S5PV210_GPH2_BASE 0xE0200C40
//gph2_6
void lightup_led()
{
	int d = readl(S5PV210_GPH2_BASE);
	d &= ~0x0F000000;
	d |= 0x01000000;
	writel(d,S5PV210_GPH2_BASE);
	d = readl(S5PV210_GPH2_BASE + 4);
	d |= 1<<6;
	writel(d,S5PV210_GPH2_BASE + 4);
}
int check_recovery_key()
{
	return !!(readl(0xE0200284) & (1<<5));	//check if vol+ key pressed
}

//check if booted from usb
//the usbboot loader will fill the memory offset 0x20000000 and 0x30000000 with 0x55555555
int check_usbboot()
{
	if(readl(0x20000000) == 0x55555555 &&
		readl(0x30000000) == 0x55555555){
		return 1;
	}else{
		return 0;
	}
}
extern int check_special_sdcard();
int special_boot = 0;
int check_special_boot()
{
	init_gpiokey();
	struct _boot_msg* bmsg = (struct _boot_msg*)BOOT_MSG_ADDR;
	int bt = 0;
	if(bmsg->magic == BOOT_MSG_MAGIC){
		if(bmsg->boot_type == BOOT_TYPE_BOOTLOADER){
			bt = 1;
			bmsg->boot_type = BOOT_TYPE_NORMAL;
		}
	}

	if(check_usbboot() || bt || check_special_sdcard())
		special_boot = 1;
	else
		special_boot = 0;
	return special_boot;
...
int fastboot_preboot(void)
{
	...
	if(check_recovery_key()){
		struct _boot_msg* bmsg = (struct _boot_msg*)BOOT_MSG_ADDR;
		bmsg->boot_type = BOOT_TYPE_RECOVERY;
	}
	//qdk++S
