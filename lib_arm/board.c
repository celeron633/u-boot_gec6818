...
#if defined(CONFIG_SMDKC110) && defined(CONFIG_FRIENDLYELEC)
	int omr_reg = OMR_REG;
	int inf_reg0_reg = INF_REG0_REG;

	if (inf_reg0_reg == 0x11111111) {
		s5pc110_chip = S5PC110_CHIP_TYPE_D;
	} else if (inf_reg0_reg == 0x22222222) {
		s5pc110_chip = S5PC110_CHIP_TYPE_N;
	} else {
		s5pc110_chip = S5PC110_CHIP_TYPE_INVALID;
	}
#endif
...
//******************************//
...
#if defined(CONFIG_SMDKC110) && defined(CONFIG_FRIENDLYELEC)
	/* adjust fuse_addr to get 384M-10M(LCD framebuffer) space for rootfs.img */
	if (s5pc110_chip == S5PC110_CHIP_TYPE_D) {
		setenv("fuse_addr", "40000000");
		printf("Chip Type: D\n");
	} else if (s5pc110_chip == S5PC110_CHIP_TYPE_N) {
		setenv("fuse_addr", "38000000");
		printf("Chip Type: N\n");
	}

	if (omr_reg == 0x9) {
		setenv("bootdev", "onenand");
	} else if (omr_reg == 0xd) {
		setenv("bootdev", "sd");
	} else {
	...
	unsigned char Lcd;
	unsigned short LcdFirmwareVersion;

	// get LCD type
	InitOneWire();
	if (!GetInfo(&Lcd, &LcdFirmwareVersion)) {
		Lcd = LCD_BAD;
		LcdFirmwareVersion = 0;
		printf("WARNING: unknown LCD type\n");
	}
	InitLCD(Lcd, 0);
	if (Lcd > 0 && LcdNameFromNumber(Lcd) != NULL) {
		setenv("lcd", LcdNameFromNumber(Lcd));
	}
#endif /* CONFIG_SMDKC110 */
